#!/bin/bash
#Alert Script for eh1-nhscs-laps01 - 10 Minute Intervals
if ping -c 1 10.162.150.17 > /dev/null
  then
    ...
    echo 1 > /cachet-scripts/status/eh1-nhscs-laps01-S2.txt
    dc01stat1=$(cat /cachet-scripts/status/eh1-nhscs-laps01-S1.txt)
    if [ $dc01stat1 = "1" ];
      then
        curl -H "X-Cachet-Token: 0dKkef9ucNbQ0mGbxGkZ" -X PUT -d 'id=18&status=2' http://eh1-nhscs-cas01.ad.nhscs.net/api/v1/components/
      else
        exit
    fi
fi
