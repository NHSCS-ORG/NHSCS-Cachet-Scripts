#!/bin/bash
#Alert Script for nhscs-eh3 - 15 Minute Intervals
if ping -c 1 10.162.150.52 > /dev/null
  then
    ...
    echo 1 > /cachet-scripts/status/nhscs-eh3-S3.txt
    dc01stat1=$(cat /cachet-scripts/status/nhscs-eh3-S1.txt)
    dc01stat2=$(cat /cachet-scripts/status/nhscs-eh3-S2.txt)
    if [ $dc01stat1 = "1" ];
      then
        if [ $dc01stat2 = "1" ];
          then
            curl -H "X-Cachet-Token: 0dKkef9ucNbQ0mGbxGkZ" -X PUT -d 'id=9&status=3' http://eh1-nhscs-cas01.ad.nhscs.net/api/v1/components/
          else
            :
        fi
      else
