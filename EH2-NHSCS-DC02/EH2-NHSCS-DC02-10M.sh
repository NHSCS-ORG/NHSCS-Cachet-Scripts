#!/bin/bash
#Alert Script for eh2-nhscs-dc02 - 10 Minute Intervals
if ping -c 1 10.162.150.11 > /dev/null
  then
    ...
    echo 1 > /cachet-scripts/status/eh2-nhscs-dc02-S2.txt
    dc01stat1=$(cat /cachet-scripts/status/eh2-nhscs-dc02-S1.txt)
    if [ $dc01stat1 = "1" ];
      then
        curl -H "X-Cachet-Token: 0dKkef9ucNbQ0mGbxGkZ" -X PUT -d 'id=13&status=2' http://eh1-nhscs-cas01.ad.nhscs.net/api/v1/components/
      else
        exit
    fi
fi
