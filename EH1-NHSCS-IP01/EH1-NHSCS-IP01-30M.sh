#!/bin/bash
#Alert Script for eh1-nhscs-ip01 - 30 Minute Intervals
if ping -c 1 10.162.150.12 > /dev/null
  then
    ...
    dc01stat4=$(cat /cachet-scripts/status/eh1-nhscs-ip01-S4.txt)
    if [ $dc01stat1 = "1" ];
      then
      ...
      then
        if [ $dc01stat2 = "1" ];
          then
            if [ $dc01stat3 = "1" ];
              then
                curl -H "X-Cachet-Token: 0dKkef9ucNbQ0mGbxGkZ" -X PUT -d 'id=14&status=4' http://eh1-nhscs-cas01.ad.nhscs.net/api/v1/components/1
              else
                :
            fi
          else
